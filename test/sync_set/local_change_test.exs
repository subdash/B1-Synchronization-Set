defmodule SyncSet.LocalChangeTest do
  ...
  setup %{tmp_dir: tmp_dir} do
    ...
    lc_opts = [index: idx_name, control: self(), sync_dir: sync_dir, name: lc_name]
  ...
  test "new content -> write + announce", %{
    ...
    # LocalChange keys the Index by the sync_dir-relative path, not the absolute one
    key = Path.relative_to(file_path, sync_dir)
    ...
    assert_receive {:local_change, ^key}, 2000
    %Entry{checksum: actual_checksum} = Index.get(idx_name, key)
  ...
  test "echo dropped", %{lc_name: lc_name, sync_dir: sync_dir, idx_name: idx_name} do
    ...
    # Write the file to the index under its relative key
    Index.local_write(idx_name, key, checksum, size)
    # Retrieve current vector fore file path
    %Entry{vector: old_vector} = Index.get(idx_name, key)
    send(lc_name, {:stable, file_path})
    ...
    %Entry{vector: new_vector} = Index.get(idx_name, key)
  ...
  test "delete live file -> tombstone + announce", %{
    ...
    file_path = Path.join(sync_dir, "file.txt")
    key = Path.relative_to(file_path, sync_dir)
    # Create an entry in the index under its relative key
    Index.local_write(idx_name, key, "abcd1234", 1)
    # Assert entry is not yet deleted
    %Entry{deleted: deleted_before} = Index.get(idx_name, key)
    ...
    assert_receive {:local_change, ^key}, 2000
    %Entry{deleted: deleted_after} = Index.get(idx_name, key)
  ...
  test "delete absent file -> nothing", %{
    ...
    assert nil == Index.get(idx_name, key)
    refute_receive {:local_change, _}
  end
  ...
  test "delete already-deleted file -> nothing", %{
    ...
    %Entry{vector: before_vector} = Index.get(idx_name, key)
    # Second delete
    send(lc_name, {:deleted, file_path})
    # Sync state and assert no second deletion announced
    :sys.get_state(lc_name)
    %Entry{vector: after_vector} = Index.get(idx_name, key)
